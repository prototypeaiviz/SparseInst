...
def resize_img(img, short_length=640, long_length=864):
    ...
    image_size = (h, w)
    # Logic for calculating new dimensions
    if h > w:
        new_h = int(h / w * short_length)
        if new_h > long_length:
            new_w = int(w / h * long_length)
            new_h = long_length
        else:
            new_w = short_length
    else:
        new_w = int(w / h * short_length)
        if new_w > long_length:
            new_h = int(h / w * long_length)
            new_w = long_length
        else:
            new_h = short_length
    # OpenCV resize (Note: dsize is (width, height))
    resized_img = cv2.resize(working_img,
                             (new_w, new_h),
                             interpolation=cv2.INTER_LINEAR)

    return resized_img,image_size
...
def main():
    ...
    folder_containing_onnx_json= "/home/aiviz05/Projects/SparseInst/output/sparse_inst_r50_base.onnx"
    session_options.graph_optimization_level = ort.GraphOptimizationLevel.ORT_DISABLE_ALL
    session_options.intra_op_num_threads = 1
    session_options.inter_op_num_threads = 1
    ort_session = ort.InferenceSession(folder_containing_onnx_json,providers=providers)
    resized_image, image_size = resize_img(img.data, short_length=640, long_length=640)
    ...
    container[:h, :w, :] = image
    image = container.transpose( 2, 0, 1)
    ...
    # final_masks = postprocess_masks(masks, img.data.shape[0], img.data.shape[1], h, w)
